...
void MainWindow::slot_vertical_splitter()
{
	//要分割的Widget
...
void MainWindow::slot_horizontal_splitter()
{
	//要分割的Widget
	QWidget* splitter_widget = focusWidget();
	if (splitter_widget == nullptr)
...
void MainWindow::slot_delete_splitter()
{
	QWidget* splitter_widget = focusWidget();
	if (splitter_widget == nullptr)
		return;
...
void MainWindow::slot_add_label()
{
...
void MainWindow::init_chart()
{
	...
	QList<double> bar_list1{ 1,24,5,7,9,10 };
	bar->add_bar(bar_list, "1");
	bar->add_bar(bar_list1, "2");
...
ThirdDData(3.1885, 8.0106, 9.62) 
	...
	radarlist << "一月" << "二月" << "三月" << "四月" << "五月";
	radar->set_radar(radarlist);
	QList<double> radar1{ 1, 2, 3, 4, 5 };
	radar->add_radar(radar1, "1");
	...
	QList<double> histogram_list1{ 0.11,0.22,0.33,0.555,0.77,8 ,27};
